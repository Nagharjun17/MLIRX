...
static Value insertAllocAndDealloc(MemRefType type, Location loc,
                                   PatternRewriter &rewriter) {
  // Create the alloc at the current insertion point, then move it to the block
  // entry so later loads/stores can dominate it.
  auto alloc = rewriter.create<memref::AllocOp>(loc, type);
  alloc->moveBefore(&alloc->getBlock()->front());

  // Create the dealloc and move it to the end of the block (Toy has no CFG).
  auto dealloc = rewriter.create<memref::DeallocOp>(loc, alloc);
  dealloc->moveBefore(&dealloc->getBlock()->back());

  // Return the SSA value (memref) produced by the alloc.
...
//===----------------------------------------------------------------------===//
// ToyToAffine Conversion Patterns: Max operation
//===----------------------------------------------------------------------===//
...
struct MaxOpLowering : public OpConversionPattern<toy::MaxOp> {
  ...
  matchAndRewrite(toy::MaxOp op, OpAdaptor adaptor, ConversionPatternRewriter &rewriter) const final {
    ...
    auto b = adaptor.getOperands()[1];

    llvm::errs() << "MaxOpLowering ptrs: a=" << a.getAsOpaquePointer()
                << " b=" << b.getAsOpaquePointer() << "\n";

    llvm::errs() << "defops: a=" << a.getDefiningOp()
                << " b=" << b.getDefiningOp() << "\n";

    // Optional, very handy:
    if (a.getDefiningOp()) { llvm::errs() << "A def:\n"; a.getDefiningOp()->dump(); }
    if (b.getDefiningOp()) { llvm::errs() << "B def:\n"; b.getDefiningOp()->dump(); }
...
struct ConstantOpLowering : public OpConversionPattern<toy::ConstantOp> {
  ...
  LogicalResult matchAndRewrite(toy::ConstantOp op, OpAdaptor adaptor,
    ...
    // Allocate the result buffer for this specific constant.
    auto tensorType = llvm::cast<RankedTensorType>(op.getType());
    auto memRefType = convertTensorToMemRef(tensorType);
    Value alloc = insertAllocAndDealloc(memRefType, loc, rewriter);

    // Pre-build index constants up to the largest dimension.
    auto valueShape = memRefType.getShape();
    SmallVector<Value, 8> constantIndices;
    if (!valueShape.empty()) {
      for (int64_t i = 0, e = *llvm::max_element(valueShape); i < e; ++i)
        constantIndices.push_back(
    ...
    // Recursively store all elements of the dense attribute into the memref.
    SmallVector<Value, 2> indices;
    auto valueIt = constantValue.value_begin<FloatAttr>();

    std::function<void(uint64_t)> storeElements = [&](uint64_t dim) {
      if (dim == valueShape.size()) {
        // Materialize the scalar constant at this position and store it.
        Value cst = rewriter.create<arith::ConstantOp>(loc, *valueIt++);
        rewriter.create<affine::AffineStoreOp>(loc, cst, alloc, indices);
        return;
      }
    ...
    // Replace this toy.constant with the *exact* memref value allocated above.
    rewriter.replaceOp(op, alloc);
    return success();
  }
};
...
void ToyToAffineLoweringPass::runOnOperation() {
  ...
  patterns.add<AddOpLowering, ConstantOpLowering, FuncOpLowering, MulOpLowering,
      ...
      &getContext());
  mlir::toy::populateToyCustomLowerings(patterns);
